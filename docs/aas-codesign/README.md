...
1. Place the following configuration files in `~/.aas-codesign`
   - [`acs.json`](./acs.json)
   - [`azure.json`](./azure.json)
...
3. Place the following script in `~/.aas-codesign`:
   - [`aas-codesign-credential.ps1`](./aas-codesign-credential.ps1)  
     Windows *(Powershell)*  
   ...
   - [`aas-codesign-credential.sh`](./aas-codesign-credential.sh)  
...
6. If this credential helper script is found by the Post Build Script of the Xojo Example Project in `~/.aas-codesign`, it will pick up the Azure Client Secret from the secret storage by calling the script
